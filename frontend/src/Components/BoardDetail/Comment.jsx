...
const Comment = ({ comment, onAddComment, onUpdateComment, onDeleteComment, onAddLikeComment, isChild }) => {
                ...
                <div className={style.comment}>
                            ...
                            <div className={style.comment_left_1_1_author}>
                                <p>
                                    <Link to={`/profile/${comment.author}/`}>
                                        {comment.author}
                                        <sup style={{ backgroundColor: getButtonColor(comment.author_mbti) }}>
                                            {comment.author_mbti.toUpperCase()}
                                        </sup>
                                    </Link>
                                </p>
                    ...
                    <div className={style.comment_right}>
                        <div className={style.comment_right_button}>
                            ...
                            {!isChild && canReply() && <button onClick={() => handleInputMode('reply')}>댓글</button>}
                            {isSameUser(comment) && (
                                <>
...
const CommentList = ({ comments, commentCount, onAddComment, onUpdateComment, onDeleteComment, onAddLikeComment }) => {
    ...
    const isMbtiInFilter = (comment) => {
        return (
            ...
            filterMbtiList.some((e) => e.toLowerCase() === comment.author_mbti.toLowerCase())
        )
    }
...
const CommentInput = ({ post, onAddComment, parentCommentId }) => {
    ...
    const notIncludedInMbtiList = () => {
        console.log(post.mbti, currentUser['mbti_type'])
        const include = post.mbti.some((e) => e.toLowerCase() === currentUser['mbti_type'].toLowerCase())
        return currentUser['mbti_type'] !== null && !include
    }
...
const CommentBox = ({
    ...
    useEffect(() => {}, [post, commentCount, comments])
